...
\begin{frame}{Sorting and searching}
    ...
    \bi
        \item Very common operations:
            \bi
                \item Sorting an array \onslide<2->{- {\color{vhilight}sort(arr.begin(), arr.end())}}
                \item Searching an unsorted array \onslide<2->{- {\color{vhilight}find(arr.begin(), arr.end(), x)}}
                \item Searching a sorted array \onslide<2->{- {\color{vhilight}lower\_{}bound(arr.begin(), arr.end(), x)}}
            \ei
...
\tikzset{
  treenode/.style = {align=center, inner sep=0pt, text centered,
    font=\sffamily},
  vertex/.style = {treenode, circle, white, font=\sffamily\bfseries\tiny, draw=white, text width=1.8em},% arbre rouge noir, noeud noir
  rvertex/.style = {treenode, circle, white, font=\sffamily\bfseries\tiny, draw=red, text width=1.8em},% arbre rouge noir, noeud noir
}
            ...
            \bi
                \item Find $k$th smallest element
                    \bi
                        ...
                        \item If $k = m+1$, we found it
                        \item If $k \leq m$, look for the $k$th smallest element in the left subtree
                        \item If $k > m+1$, look for the $m-k-1$st smallest element in the right subtree
                    \ei
                \item Example: $k=11$
            \ei
        \end{column}%
